...
internal sealed class UpdateStockTypeCommandHandler(IStockTypeRepository stockTypeRepository,IUnitOfWork unitOfWork): IRequestHandler<UpdateStockTypeCommand>
{
    ...
    {
        StockType stockType = await stockTypeRepository.GetByExpressionWithTrackingAsync(p => p.Id == request.Id,cancellationToken);
        ...
        if (stockType.Name != request.Name)
        {
            ...
            if (isNameExists)
            {
                throw new ArgumentException("stok türü adı daha önce kullanılmıştır");
            }
        }
